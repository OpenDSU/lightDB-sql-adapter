...
if (isMainThread) {
    class SQLAdapter {
        constructor(type) {
            ...
            // Don't create a worker pool with workerData containing the pool
            this.workerPool = syndicate.createWorkerPool({
                ...
                workerOptions: {
                    workerData: {
                        type: this.type,
                        // Only send connection config, not the pool itself
        ...
        _executeTask(taskName, args) {
            return new Promise((resolve, reject) => {
                try {
                    ...
                    this.workerPool.addTask({taskName, args: safeArgs}, (err, result) => {
                        if (err) {
    ...
    async function initializePool() {
        if (!pool) {
            pool = new Pool(workerData.config);
    ...
    parentPort.on('message', async (message) => {
        ...
        try {
            ...
            result = await strategy.executeTask(pool, taskName, args);
